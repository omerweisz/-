...
# הגדרות דף
st.set_page_config(page_title="מערכת OSINT v22.0 - ניטור מקורות מורחב", layout="wide")

def get_israel_time():
    return datetime.utcnow() + timedelta(hours=2)

# רשימת המקורות המלאה (ה"עיניים")
SOURCES_FULL = {
    "11": "כאן 11", "12": "חדשות 12", "13": "חדשות 13", "14": "ערוץ 14",
    "ynet": "ynet", "פקע\"ר": "פיקוד העורף", "גל\"צ": "גלי צה\"ל", 
    "צה\"ל": "אתר צה\"ל", "telegram": "מקורות טלגרם", "adsb": "ניטור כלי טיס"
}

# --- ניהול זיכרון (Session State) ---
# בדיקה ומניעת KeyError: מוודא שכל המפתחות קיימים בזיכרון
if 'active_sources' not in st.session_state:
    st.session_state['active_sources'] = {key: False for key in SOURCES_FULL.keys()}
else:
    # עדכון מפתחות חסרים במידה והוספנו מקורות חדשים בקוד
...
def enhanced_eye_scan(selected_region):
    isr_now_str = get_israel_time().strftime('%H:%M')
    
    # סימולציה של זיהוי אירוע
...
# --- ממשק משתמש ---
st.markdown("<h1 style='text-align: right;'>🛰️ חדר מצב OSINT: ניטור מקורות מורחב</h1>", unsafe_allow_html=True)

# הצגת ה"עיניים" (המקורות)
...
for i, (key, name) in enumerate(SOURCES_FULL.items()):
    ...
    n_cols[i].markdown(f"<div style='text-align:center; border:1px solid {color}; border-radius:5px; padding:8px; background-color: rgba(0,0,0,0.1);'>"
                       f"<b style='font-size:10px;'>{name}</b><br>"
                       f"<span style='color:{color}; font-size:16px;'>●</span></div>", unsafe_allow_html=True)
...
with col_side:
    ...
    current_val = enhanced_eye_scan(region)
...
if st.button("בצע סנכרון וסריקה מורחבת 🔄"):
    st.rerun()
