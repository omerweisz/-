...
def auto_refresh_hamaal():
    ...
    for idx, key in enumerate(all_keys):
        s = source_results.get(key, "GREEN")
        c = {"ALARM_100": "#ff1a1a", "LAUNCH_95": "#ff4400", "RELEASE": "#00ff00", "GREEN": "#00ff00"}.get(s, "#00ff00")
        with cols[idx % 7]:
            st.markdown(f"""<div style="text-align: center; margin-bottom: 10px;"><div style="width: 6px; height: 6px; background: {c}; border-radius: 50%; display: inline-block;"></div><br><span style="font-size:7px; color: #555;">{key}</span></div>""", unsafe_allow_html=True)
