...
# הגדרות דף
st.set_page_config(page_title="חמ\"ל עבר הירקון - IRAN WATCH", layout="wide")
...
def check_multi_source_osint():
    ...
    # יעדים אסטרטגיים
    target_zones = ["תל אביב", "גוש דן", "המרכז", "עבר הירקון", "גלילות", "רמת אביב"]
    ...
    for url in sources:
        try:
            ...
            for item in root.findall('.//item'):
                ...
                # בדיקה רק ל-15 דקות האחרונות
                if 0 <= (now - pub_date).total_seconds() / 60 <= 15:
                    
                    # לוגיקה לאיראן/מרכז: אם יש "איראן" וגם יעד במרכז או מילת שיגור
                    is_iran = "איראן" in title
                    is_target_center = any(tz in title for tz in target_zones)
                    is_action = any(cw in title for cw in critical_words)

                    # תרחיש א' - איראן משגרת למרכז: 100%
                    if is_iran and (is_target_center or is_action):
                        return "IRAN_LAUNCH", title
                    
                    # תרחיש ב' - אזעקה רגילה בתל אביב: 99.8%
                    if any(cw in title for cw in critical_words) and any(tz in title for tz in target_zones):
                        return "RED_ALERT", title

                    # תרחיש ג' - אזעקה בשרון: 65%
...
def auto_refresh_hamaal():
    ...
    fig.update_layout(margin=dict(l=0, r=0, t=5, b=0), height=150, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', xaxis=dict(visible=False, fixedrange=True), yaxis=dict(visible=False, fixedrange=True, range=[0, 110]), showlegend=False, dragmode=False)
    ...
    for idx, key in enumerate(all_keys):
        with cols[idx % 7]:
            st.markdown(f"""<div style="text-align: center; margin-bottom: 10px;"><div style="width: 6px; height: 6px; background: {color}; border-radius: 50%; display: inline-block; box-shadow: 0 0 8px {color};"></div><br><span style="font-size:8px; color: #FFFFFF; font-weight: bold; opacity: 0.8;">{key}</span></div>""", unsafe_allow_html=True)
