...
# יצירת נתוני הגרף ברזולוציה של דקה (120 דקות קדימה)
times = []
values = []

for i in range(120):
    future_time = now + timedelta(minutes=i) # שינוי ל-1 במקום 10
    times.append(future_time)
    
    stat_risk = HOURLY_STATS[future_time.hour]
    
    if i < 60 and live_modifier > 0: # דעיכה לאורך 60 דקות במקרה של אירוע
...
with col_graph:
    ...
    fig.update_layout(
        ...
        xaxis=dict(fixedrange=True, tickformat='%H:%M', nticks=20), 
...
with col_stat:
    st.metric("רמת סיכון נוכחית", f"{current_total_risk:.1f}%", 
              delta="מצב פעיל" if is_alert else "שגרה", 
