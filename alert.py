...
# הגדרות דף
st.set_page_config(page_title="חמ\"ל עבר הירקון - STRATEGIC V6", layout="wide")
...
def auto_refresh_hamaal():
    ...
    elif status_type == "SINGLE_SOURCE": color = "#ffaa00"
    else: color = "#ff1a1a"

    # עיצוב הכותרת עם טקסט לבן ברור
    st.markdown(f"""
        <div style="text-align: center; padding: 20px; border: 1px solid {color}44; border-radius: 15px; background: rgba(0,0,0,0.5); box-shadow: 0 0 20px {color}15;">
            <p style="color: #FFFFFF; font-size: 10px; margin: 0; letter-spacing: 3px; font-weight: bold; opacity: 0.9;">UNIT: EVER HAYARKON | STRATEGIC DEP</p>
            ...
            <div style="color: #FFFFFF; font-size: 13px; font-family: 'JetBrains Mono'; font-weight: bold; margin-top: 10px;">
                ...
                <span style="color: #EEEEEE; margin-left: 15px; font-size: 11px; opacity: 0.8;">| SYS: {health['NEWS']} | AIR: {health['AIR']}</span>
            </div>
        </div>
    """, unsafe_allow_html=True)
    ...
    fig.update_layout(margin=dict(l=0, r=0, t=5, b=0), height=150, paper_bgcolor='rgba(0,0,0,0)', plot_bgcolor='rgba(0,0,0,0)', xaxis=dict(visible=False), yaxis=dict(visible=False, range=[0, 110 if is_emergency else 45]))
    st.plotly_chart(fig, use_container_width=True, config={'displayModeBar': False})

    # נורות בקרה עם טקסט לבן
        ...
        with cols[idx % 7]:
            st.markdown(f"""
                <div style="text-align: center; margin-bottom: 10px;">
                    <div style="width: 6px; height: 6px; background: {color}; border-radius: 50%; display: inline-block; box-shadow: 0 0 8px {color};"></div>
                    <br><span style="font-size:8px; color: #FFFFFF; font-weight: bold; opacity: 0.8; text-transform: uppercase;">{key}</span>
                </div>
            """, unsafe_allow_html=True)

    # לוג התראות לבן
    if st.session_state.alert_log:
        st.markdown("<div style='color: #FFFFFF; font-size: 10px; margin-top: 15px; border-top: 1px solid #333; padding-top: 8px; font-weight: bold;'>RECENT STRATEGIC LOGS:</div>", unsafe_allow_html=True)
        for entry in st.session_state.alert_log:
            st.markdown(f"<p style='color: #EEEEEE; font-size: 11px; margin: 2px 0;'><b>[{entry['time']}]</b> {entry['msg']}</p>", unsafe_allow_html=True)
